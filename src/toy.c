...
void access_toy(toy_t* self, int client_id){ //ATENÇÃO, remover o client_id depois
    ...
    pthread_mutex_lock(&self->toy_running_mutex);  // Trava o mutex para garantir acesso exclusivo à condição
    while (self->toy_isrunning) {  // Verifica o estado da condição para evitar Spurious Wakeups
        debug("Cliente [%d] entrou no while 1\n", client_id);
        pthread_cond_wait(&self->running_toy_cond, &self->toy_running_mutex);  // Espera pela liberação do brinquedo
        // vale lembrar que o pthread_cond_wait quando chamado faz um unlock no self->toy_running_mutex
        // e até que o sinal de condição válida (feito na thread do brinquedo) seja feito, a thread do cliente fica bloqueada
    }
    pthread_mutex_unlock(&self->toy_running_mutex); // Destrava o mutex relacionado à espera, significando que o brinquedo está disponível
    ...
    pthread_mutex_lock(&self->toy_ready_mutex);  // Trava o mutex para garantir acesso exclusivo à condição
    while (!self->toy_isready) {  // Verifica o estado da condição para evitar Spurious Wakeups
        debug("Cliente [%d] entrou no while 2\n", client_id);
        pthread_cond_wait(&self->ready_toy_cond, &self->toy_ready_mutex);  // Espera pela liberação do brinquedo
        // vale lembrar que o pthread_cond_wait quando chamado faz um unlock no self->toy_ready_mutex
        // e até que o sinal de condição válida (feito na thread do brinquedo) seja feito, a thread do cliente fica bloqueada
    }
    pthread_mutex_unlock(&self->toy_ready_mutex); // Destrava o mutex relacionado à espera, significando que o brinquedo está disponível
...
void *turn_on(void *args){
    ...
    while (cont < 3) {
        ...
        // toy->toy_isready = 1;   // a variável que identifica se o brinquedo está pronto é alterada para 1 (true)
        ...
        // toy->toy_isrunning = 0; // a variável que identifica se o brinquedo está em execução é alterada para 0 (false)
        toy->toy_isready = 1;   // a variável que identifica se o brinquedo está pronto é alterada para 1 (true)
        pthread_cond_broadcast(&toy->ready_toy_cond); // utilizamos o cond_broadcast para sinalizar a TODAS as threads dos clientes que o brinquedo está disponível
        cont++;
    }
...
void open_toys(toy_args *args){
    ...
    for (int i = 0; i < n_toys; i++) {
        ...
        args->toys[i]->toy_isready = 0;
        pthread_mutex_init(&args->toys[i]->toy_running_mutex, NULL); // inicializa o mutex relacionado ao controle de execução do brinquedo
        pthread_cond_init(&args->toys[i]->running_toy_cond, NULL); // inicializa a condição relacionada à execução do brinquedo
        pthread_mutex_init(&args->toys[i]->toy_ready_mutex, NULL); // inicializa o mutex relacionado ao controle de execução do brinquedo
        pthread_cond_init(&args->toys[i]->ready_toy_cond, NULL); // inicializa a condição relacionada à execução do brinquedo
...
void close_toys(){
    ...
    for (int i = 0; i < n_toys; i++) {
        ...
        pthread_mutex_destroy(&array_toys[i]->toy_running_mutex); // Destroy o mutex de controle da execução do brinquedo
        pthread_cond_destroy(&array_toys[i]->running_toy_cond); // Destroy a condição de execução do brinquedo 
        pthread_mutex_destroy(&array_toys[i]->toy_ready_mutex); // Destroy o mutex de controle da execução do brinquedo
        pthread_cond_destroy(&array_toys[i]->ready_toy_cond); // Destroy a condição de execução do brinquedo 
    }
